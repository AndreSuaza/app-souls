...
export const metadata: Metadata = {
  ...
  description:
    "Participa en los torneos oficiales de Souls In Xtinction TCG y enfréntate a los mejores jugadores. Consulta fechas, inscripciones, reglas y premios.",
  openGraph: {
    title:
...
export default async function EventosPage({ searchParams }: Props) {
  ...
  const { tournaments } = await getTournamentsPagination({
    page: page2,
    take,
    types: ["Tier 1", "Tier 2", "Tier 3"],
  });
  const { tournaments: tournamentsT1 } = await getTournamentsPagination({
    types: ["Tier 4"],
  });

  return (
    <>
      <Title title="Torneos" className="mb-3 lg:mb-6" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 xl:mx-10 m-2">
        ...
        <div className="md:ml-4">
          <div className="bg-gray-200 px-4 rounded-lg pb-4">
            <h2 className="text-4xl font-bold pt-4 mb-6 uppercase">
              Torneos Semanales
            </h2>
            <TournamentGrid tournaments={tournamentsT1} short={true} />
          </div>
        </div>
      </div>
    </>
