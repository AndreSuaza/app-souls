          ...
          <ul className="grid gap-4 pb-10 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {cards.map((card) => (
              <li key={card.id}>
                <Link
                  href={`/boveda/${card.id}`}
                  className="grid grid-cols-[140px_1fr] sm:grid-cols-[160px_1fr] gap-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-purple-300 dark:border-tournament-dark-border dark:bg-tournament-dark-surface dark:hover:border-purple-400/40"
                >
                  <Image
