...
const LEGACY_AVATAR_MAP: Record<string, string> = {
  ...
  erik:
    "souls/profile/avatars/erik-48100217-48c1-4330-ba68-3cc692b668f2.webp",
  estraker:
  ...
  toro:
    "souls/profile/avatars/toro-1eed2c59-39b7-4670-a096-9fb54ed31d3d.webp",
  voluntad:
...
const normalizeAvatarValue = (value?: string | null) => {
  if (!value) return `souls/profile/avatars/${DEFAULT_AVATAR}.webp`;
  if (isHttpUrl(value)) return value;
  if (value.startsWith("souls/")) return value;
  if (value.startsWith("/")) return value.slice(1);
  if (value.includes("/")) return value;
  if (LEGACY_AVATAR_MAP[value]) return LEGACY_AVATAR_MAP[value];
  return `souls/profile/avatars/${value}.webp`;
};
