...
function swissPairingNoRepeat(origPlayers: Player[]): Pair[] {
  ...
  if (players.length % 2 === 1) {
    ...
    if (!byePlayer) {
      players.sort(
        (a, b) => a.points - b.points || a.name.localeCompare(b.name)
      );
      byePlayer = players[0];
    }
    ...
    pairs.push({
      player1: byePlayer!,
      player2: { name: "BYE", points: 0, rivals: [] },
    }); // null representa Bye
  }
  ...
  function tryPair(pool: Player[]): Pair[] | null {
    ...
    const candidates = pool.slice(1).sort((x, y) => {
    ...
    for (const cand of candidates) {
      ...
      const newPool = pool.filter(
        (p) => p.name !== first.name && p.name !== cand.name
      );
