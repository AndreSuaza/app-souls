...
export const CardFinderLabLocal = ({
  propertiesCards,
  cols = 6,
  onSearch,
}: Props) => {
  ...
  const [properties] = useState({
    types: propertiesCards.types.map((prop: SelectProps) => {
    ...
    archetypes: propertiesCards.archetypes.map((prop: SelectProps) => {
      return { label: prop.name, value: prop.id };
    }),
    keywords: propertiesCards.keywords.map((prop: SelectProps) => {
      return { label: prop.name, value: prop.id };
    }),
    products: propertiesCards.products.map((prop: SelectProps) => {
      return { label: `${prop.name} [${prop.id}]`, value: prop.id };
    }),
    rarities: propertiesCards.rarities.map((prop: SelectProps) => {
      return { label: `${prop.name}`, value: prop.id };
    }),
  });

  const { types, others, archetypes, keywords, products, rarities } =
    properties;
  ...
  const buildFilters = (
    ...
    if (filters.types.length > 0)
      nextFilters.types = getFilterValues(filters.types);
    if (filters.force.length > 0)
      nextFilters.forces = getFilterValues(filters.force);
    if (filters.archetypes.length > 0)
      nextFilters.archetypes = getFilterValues(filters.archetypes);
    if (filters.products.length > 0)
      nextFilters.products = getFilterValues(filters.products);
    if (filters.cost.length > 0)
      nextFilters.costs = getFilterValues(filters.cost);
    if (filters.defense.length > 0)
      nextFilters.defenses = getFilterValues(filters.defense);
    if (filters.keywords.length > 0)
      nextFilters.keywords = getFilterValues(filters.keywords);
    if (filters.rarities.length > 0)
      nextFilters.rarities = getFilterValues(filters.rarities);
    if (filters.limit.length > 0)
      nextFilters.limit = getFilterValues(filters.limit);
        ...
        <Form>
          <div>
            ...
            <div className="flex justify-center mx-2">
              <button
                type="submit"
                className="btn-primary w-full md:w-[200px] "
              >
                Buscar
              </button>
            </div>
