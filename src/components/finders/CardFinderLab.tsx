...
export const CardFinderLab = ({ propertiesCards, cols = 6 }: Props) => {
  ...
  const searchCards = (filters: PropertiesFiltersFinderLab) => {
    ...
    if (filters.text && filters.text !== "")
      query += "&text=" + capitalizeWords(filters.text);
    if (filters.types.length > 0)
      query += "&types=" + getFilterValues(filters.types);
    if (filters.force.length > 0)
      query += "&forces=" + getFilterValues(filters.force);
    if (filters.archetypes.length > 0)
      query += "&archetypes=" + getFilterValues(filters.archetypes);
    if (filters.products.length > 0)
      query += "&products=" + getFilterValues(filters.products);
    if (filters.cost.length > 0)
      query += "&costs=" + getFilterValues(filters.cost);
    if (filters.defense.length > 0)
      query += "&defenses=" + getFilterValues(filters.defense);
    if (filters.keywords.length > 0)
      query += "&keywords=" + getFilterValues(filters.keywords);
    if (filters.rarities.length > 0)
      query += "&rarities=" + getFilterValues(filters.rarities);
    if (filters.limit.length > 0)
      query += "&limit=" + getFilterValues(filters.limit);
